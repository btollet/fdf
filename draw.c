...
void	draw_line(t_data fdf_data, t_point *point1, t_point *point2)
{
	...
	p1.x = ((point2->x + fdf_data.decal_x) - (point1->x + fdf_data.decal_x)) / fdf_data.div_x;
	p1.y = (point2->y + fdf_data.decal_y) / fdf_data.div_y - (point1->y + fdf_data.decal_y) / fdf_data.div_y;
	inc.x = (p1.x > 0) ? 1 : -1;
	inc.y = (p1.y > 0) ? 1 : -1;
	p1.x = abs(p1.x);
	p1.y = abs(p1.y);
